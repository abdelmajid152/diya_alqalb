...
/// Controller for Quran audio player screen
class QuranPlayerController extends GetxController {
  ...
  final suwarList = <Map<String, dynamic>>[].obs;

  // Current surah display
  String get currentName =>
      suwarList.isNotEmpty ? suwarList[currentIndex.value]['name'] ?? '' : '';
  int get currentId =>
      suwarList.isNotEmpty ? suwarList[currentIndex.value]['id'] ?? 0 : 0;
  String get currentUrl =>
      suwarList.isNotEmpty ? suwarList[currentIndex.value]['url'] ?? '' : '';
  ...
  void _initFromArguments() {
    ...
    suwarList.value = List<Map<String, dynamic>>.from(args['suwarList'] ?? []);
  ...
  void _handleTrackComplete() {
    switch (repeatMode.value) {
      ...
      case RepeatMode.all:
        ...
        if (currentIndex.value < suwarList.length - 1) {
          currentIndex.value++;
        } else {
